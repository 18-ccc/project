...
if st.session_state.page == 'main':
    ...
    # 用户输入姓名
    st.session_state.name = st.text_input("请输入您的姓名")
    ...
    if st.button('💞'):
        ...
        with st.spinner('🎑正在验证姓名...'):
            time.sleep(1)  # 模拟处理时间
            if st.session_state.name in valid_names:
    ...
    # 如果输入的名字是唐悦媛，则显示特别的祝福消息和图片
    if st.session_state.name == '唐悦媛':
        st.markdown("<h2>还有bb，生日快乐🎂🎂🎂🫶🫶🫶！！！</h2>", unsafe_allow_html=True)
